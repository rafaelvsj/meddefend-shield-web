          ...
          <div className={`transition-all duration-1000 delay-200 ${visibleElements.has('contato') ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-10'}`}>
            ...
            <form onSubmit={handleFormSubmit} className="space-y-6">
              <div>
                ...
                <Input id="nome" name="nome" type="text" required value={formData.nome} onChange={handleFormChange} className="mt-1 transition-all duration-300 focus:scale-105 bg-slate-800 border-slate-600 text-white focus:border-purple-400" />
              </div>

              <div>
                <Label htmlFor="email" className="text-gray-300">E-mail *</Label>
                <Input id="email" name="email" type="email" required value={formData.email} onChange={handleFormChange} className="mt-1 transition-all duration-300 focus:scale-105 bg-slate-800 border-slate-600 text-white focus:border-purple-400" />
              </div>

              <div>
                <Label htmlFor="whatsapp" className="text-gray-300">WhatsApp</Label>
                <Input id="whatsapp" name="whatsapp" type="tel" value={formData.whatsapp} onChange={handleFormChange} className="mt-1 transition-all duration-300 focus:scale-105 bg-slate-800 border-slate-600 text-white focus:border-purple-400" />
              </div>

              <div>
                <Label htmlFor="especialidade" className="text-gray-300">Especialidade Médica</Label>
                <Input id="especialidade" name="especialidade" type="text" value={formData.especialidade} onChange={handleFormChange} className="mt-1 transition-all duration-300 focus:scale-105 bg-slate-800 border-slate-600 text-white focus:border-purple-400" />
              </div>

              <div>
                <Label htmlFor="mensagem" className="text-gray-300">Mensagem</Label>
                <Textarea id="mensagem" name="mensagem" rows={4} value={formData.mensagem} onChange={handleFormChange} className="mt-1 transition-all duration-300 focus:scale-105 bg-slate-800 border-slate-600 text-white focus:border-purple-400" />
              </div>
          ...
          <div className={`space-y-8 transition-all duration-1000 delay-400 ${visibleElements.has('contato') ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-10'}`}>
            ...
            <Card className="p-6 hover:scale-105 transition-all duration-300 hover:shadow-lg bg-slate-800/50 border-slate-700">
              <CardContent className="p-0">
                ...
                <a href="mailto:parcerias@meddefend.tech" className="text-purple-400 hover:text-purple-300 font-medium transition-colors duration-300 hover:underline">suporte@meddefend.tech</a>
              </CardContent>
            </Card>
          </div>
        </div>
